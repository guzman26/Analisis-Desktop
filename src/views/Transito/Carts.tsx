...
const TransitoCarts = () => {
  ...
  const loadCarts = useCallback(
    async (resetPagination = false, currentNextKey?: string | null) => {
      ...
      try {
        ...
        console.error('Error al cargar carros en tránsito:', error);
      } finally {
  ...
  return (
    <div className="v2-page animate-fade-in">
      <LoadingOverlay show={loading} text="Cargando carros en tránsito…" />
      <PageHeaderV2
        title="Carros en Tránsito"
        description="Lista de carros en Tránsito."
        actions={
      ...
      {allCarts.length === 0 && !loadingMore && !loading ? (
        <EmptyStateV2
          title="No hay carros en tránsito"
          description="Cuando se muevan carros a Tránsito aparecerán en esta vista."
        />
      ) : (
        <div
          style={{
